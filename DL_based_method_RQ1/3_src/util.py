...
def tsv2dict(tsv_path):
    ...
    """
    tsv_path = tsv_path.strip()
    reader = csv.DictReader(open(tsv_path, "r", encoding="utf-8"), delimiter="\t")
    dict_list = []
    for line in reader:
        line["files"] = [
            os.path.normpath(f)
...
def get_all_source_code(start_dir):
    ...
    for dir_, dir_names, file_names in os.walk(start_dir):
        for filename in [f for f in file_names if f.endswith(".py")]:
            src_name = os.path.join(dir_, filename)
            if 'git' in src_name or 'jpg' in src_name or 'png' in src_name:
                continue
            print (src_name)
...
def helper_collections(samples, br_file_path, only_rvsm=False):
    ...
    bug_reports = tsv2dict(br_file_path) #tsv2dict("../data/Eclipse_Platform_UI.txt")

    # bug_reports = tsv2dict('D:\\Bug-Localization-using-DNN-rVSM-for-deep-learning-bugs\\data\\tensorflow+tensorflow_raw_preprocessed.tsv')
...
def topk_accuarcy(test_bug_reports, sample_dict, br2files_dict, clf=None):
    ...
    """
       ...
       # Ensure that there are enough test_bug_reports
    if len(test_bug_reports) == 0:
        return {}, 0, 0 
    ...
    for bug_report in test_bug_reports:
        ...
        relevancy_list_len = len(relevancy_list)
        for i in range(1, min(21, relevancy_list_len + 1)):
        #for i in range(1, 21):
        ...
        predicted_value = relevancy_list
        true_labels = np.isin(np.array(corresponding_files), br2files_dict[bug_id]).astype(int)
        # print("Predicted Values/Relevancy List:", predicted_value)
        # print("True Labels:", true_labels)
        mp_val = average_precision_score(true_labels, predicted_value)
        map_score.append(0 if np.isnan(mp_val) else mp_val)

        sorted_prediction_rank = np.argsort(-relevancy_list)
        sorted_prediction_value = np.array([true_labels[item] for item in sorted_prediction_rank])
    ...
    map_value = sum(map_score) / (len(test_bug_reports) - negative_total)
    mrr_value = sum(mrr_score) / (len(test_bug_reports) - negative_total)
