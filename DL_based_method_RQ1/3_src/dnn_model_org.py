...
def kfold_split_indexes(k, len_samples):
    ...
    if len_samples < k:
        raise ValueError("Number of samples must be greater than or equal to the number of folds")

    step = max(1, ceil(len_samples / k))
    ret_list = [(start, min(start + step, len_samples)) for start in range(0, len_samples, step)]
    return ret_list
...
def train_dnn(
    ...
    print(f"Shape of input data (X): {X_train.shape}")
    print(f"Number of samples in input data: {len(X_train)}")
    print(f"Shape of input data (X): {y_train.shape}")
    print(f"Number of samples in input data: {len(y_train)}")
...
def dnn_model_kfold(k=10):
    ...
    acc_dicts = Parallel()(  # Uses all cores but one
        delayed(train_dnn)(
